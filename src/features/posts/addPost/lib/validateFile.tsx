...
export function validateFile(file: File): ReactElement | null | string {
  ...
  if (file.size < MIN_FILE_SIZE.bytes) {
    return `File must be at least ${MIN_FILE_SIZE.size} KB`
  }

  if (file.size > MAX_FILE_SIZE.bytes) {
    return `The image size mustn't exceed ${MAX_FILE_SIZE.size} MB`
  }
